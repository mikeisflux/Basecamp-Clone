...
class BCWP_Notification_Service {
    ...
    public static function create( $user_id, $data ) {
        ...
        $notification_id = BCWP_Database::insert( 'notifications', $notification_data );

        // Send email if enabled
        if ( get_option( 'bcwp_enable_email_notifications' ) ) {
    ...
    public static function mark_all_as_read( $user_id ) {
        ...
        return $wpdb->query( $wpdb->prepare(
            "UPDATE " . BCWP_Database::get_table_name( 'notifications' ) . "
    ...
    public static function get_unread_count( $user_id ) {
        return BCWP_Database::count( 'notifications', array(
            'user_id' => $user_id,
            'is_read' => 0,
        ) );
    }
    ...
    private static function send_email_notification( $user_id, $notification_data ) {
        ...
        $to = $user->user_email;
        $subject = '[' . get_option( 'bcwp_company_name' ) . '] ' . $notification_data['title'];
