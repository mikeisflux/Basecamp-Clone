...
class BCWP_Chat_Endpoint extends BCWP_REST_API {
    ...
    public function get_messages( $request ) {
        ...
        $messages = BCWP_Chat::get_project_messages( $project_id, $limit );
    ...
    public function create_message( $request ) {
        ...
        if ( ! BCWP_Permission_Service::can_create_content( $project_id, 'chat' ) ) {
            return $this->error_response( __( 'Access denied.', 'basecamp-wp-pro' ), 'access_denied', 403 );
        }
        ...
        $message_id = BCWP_Chat::create_message( $message_data );

        if ( ! $message_id ) {
            return $this->error_response( __( 'Failed to send message.', 'basecamp-wp-pro' ), 'create_failed' );
        }
    ...
    public function poll_messages( $request ) {
        ...
        $messages = BCWP_Chat::get_messages_since( $project_id, $since_id );
