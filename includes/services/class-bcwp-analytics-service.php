...
class BCWP_Analytics_Service {
    ...
    private static function get_project_overview( $project_id ) {
        ...
        $total_messages = $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}bcwp_messages WHERE project_id = %d",
            $project_id
        ) );

        $total_todos = $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}bcwp_todos WHERE project_id = %d",
            $project_id
        ) );

        $completed_todos = $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}bcwp_todos WHERE project_id = %d AND is_completed = 1",
            $project_id
        ) );

        $total_events = $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}bcwp_events WHERE project_id = %d",
            $project_id
        ) );

        $total_documents = $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}bcwp_documents WHERE project_id = %d",
            $project_id
        ) );

        $total_members = $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}bcwp_project_members WHERE project_id = %d",
            $project_id
        ) );
    ...
    private static function get_activity_trend( $project_id, $days = 30 ) {
        ...
        for ( $i = $days - 1; $i >= 0; $i-- ) {
            ...
            $count = $wpdb->get_var( $wpdb->prepare(
                "SELECT COUNT(*) FROM {$wpdb->prefix}bcwp_activities
                WHERE project_id = %d
                AND created_at >= %s
                AND created_at < %s",
                $project_id,
    ...
    private static function get_todo_completion_stats( $project_id, $days = 30 ) {
        ...
        for ( $i = $days - 1; $i >= 0; $i-- ) {
            ...
            $completed = $wpdb->get_var( $wpdb->prepare(
                "SELECT COUNT(*) FROM {$wpdb->prefix}bcwp_todos
                WHERE project_id = %d
                AND is_completed = 1
                AND updated_at >= %s
                AND updated_at < %s",
    ...
    private static function get_team_activity( $project_id, $days = 30 ) {
        ...
        $results = $wpdb->get_results( $wpdb->prepare(
            ...
            FROM {$wpdb->prefix}bcwp_activities
            WHERE project_id = %d
            AND created_at >= %s
            GROUP BY user_id
        ...
        foreach ( $results as $row ) {
            $data[] = array(
                'user_id' => $row->user_id,
                'user_name' => BCWP_Auth_Service::get_user_display_name( $row->user_id ),
    ...
    private static function get_message_engagement( $project_id, $days = 30 ) {
        ...
        $results = $wpdb->get_results( $wpdb->prepare(
            ...
            FROM {$wpdb->prefix}bcwp_messages m
            LEFT JOIN {$wpdb->prefix}bcwp_comments c ON c.subject_id = m.id AND c.subject_type = 'message'
        ...
        foreach ( $results as $row ) {
            $data[] = array(
                ...
                'author' => BCWP_Auth_Service::get_user_display_name( $row->created_by ),
    ...
    public static function get_user_analytics( $user_id, $days = 30 ) {
        ...
        $messages_created = $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}bcwp_messages
            WHERE created_by = %d
            AND created_at >= %s",
            $user_id,
            $since
        ...
        $comments_made = $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}bcwp_comments
            WHERE user_id = %d
            AND created_at >= %s",
            $user_id,
            $since
    ...
    public static function get_workspace_analytics( $days = 30 ) {
        ...
        $total_projects = $wpdb->get_var(
            "SELECT COUNT(*) FROM {$wpdb->prefix}bcwp_projects"
        );

        $total_users = $wpdb->get_var(
            "SELECT COUNT(DISTINCT user_id) FROM {$wpdb->prefix}bcwp_project_members"
        );
        ...
        $completed_todos = $wpdb->get_var(
            "SELECT COUNT(*) FROM {$wpdb->prefix}bcwp_todos WHERE is_completed = 1"
        );

        $recent_activity = $wpdb->get_var( $wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}bcwp_activities WHERE created_at >= %s",
            $since
        ) );
        ...
        if ( $most_active_project ) {
            $project = BCWP_Project::get( $most_active_project->project_id );
