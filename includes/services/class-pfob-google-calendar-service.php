...
class PFOB_Google_Calendar_Service {

    /**
     * Get Google OAuth authorization URL.
     */
    public static function get_authorization_url( $user_id ) {
        $client_id = get_option( 'pfob_google_client_id' );
        ...
        update_user_meta( $user_id, 'pfob_google_oauth_state', $state );
    ...
    public static function handle_oauth_callback( $code, $state, $user_id ) {
        // Verify state token
        $stored_state = get_user_meta( $user_id, 'pfob_google_oauth_state', true );
        ...
        $client_id = get_option( 'pfob_google_client_id' );
        $client_secret = get_option( 'pfob_google_client_secret' );
        $redirect_uri = self::get_redirect_uri();
        ...
        if ( isset( $body['access_token'] ) ) {
            // Store tokens
            update_user_meta( $user_id, 'pfob_google_access_token', $body['access_token'] );

            if ( isset( $body['refresh_token'] ) ) {
                update_user_meta( $user_id, 'pfob_google_refresh_token', $body['refresh_token'] );
            }

            // Store expiration
            $expires_at = time() + ( $body['expires_in'] ?? 3600 );
            update_user_meta( $user_id, 'pfob_google_token_expires_at', $expires_at );

            // Clear state token
            delete_user_meta( $user_id, 'pfob_google_oauth_state' );
    ...
    private static function refresh_access_token( $user_id ) {
        $refresh_token = get_user_meta( $user_id, 'pfob_google_refresh_token', true );
    ...
    private static function get_valid_access_token( $user_id ) {
        $access_token = get_user_meta( $user_id, 'pfob_google_access_token', true );
        $expires_at = get_user_meta( $user_id, 'pfob_google_token_expires_at', true );
    ...
    public static function sync_event_to_google( $event, $user_id ) {
        ...
        $calendar_id = get_user_meta( $user_id, 'pfob_google_calendar_id', true );
    ...
    public static function delete_event_from_google( $event_id, $user_id ) {
        ...
        $calendar_id = get_user_meta( $user_id, 'pfob_google_calendar_id', true ) ?: 'primary';
        $url = "https://www.googleapis.com/calendar/v3/calendars/{$calendar_id}/events/{$google_event_id}";
        ...
        PFOB_Event::update( $event_id, array(
            'metadata' => json_encode( $metadata )
        ) );
    ...
    public static function disconnect( $user_id ) {
        delete_user_meta( $user_id, 'pfob_google_access_token' );
        delete_user_meta( $user_id, 'pfob_google_refresh_token' );
        delete_user_meta( $user_id, 'pfob_google_token_expires_at' );
        delete_user_meta( $user_id, 'pfob_google_calendar_id' );
        delete_user_meta( $user_id, 'pfob_google_oauth_state' );
    }
    ...
    public static function get_settings( $user_id ) {
        return array(
            ...
            'auto_sync' => get_user_meta( $user_id, 'pfob_google_auto_sync', true ) === '1',
        );
    }
    ...
    public static function update_settings( $user_id, $settings ) {
        if ( isset( $settings['auto_sync'] ) ) {
            update_user_meta( $user_id, 'pfob_google_auto_sync', $settings['auto_sync'] ? '1' : '0' );
        }
    }
}
