...
class PFOB_Email_Service {

    public static function send( $to, $subject, $message, $headers = array() ) {
        // Add default headers
        $default_headers = array(
            'Content-Type: text/html; charset=UTF-8',
            'From: ' . get_option( 'pfob_company_name' ) . ' <' . get_option( 'admin_email' ) . '>',
        );
    ...
    public static function send_invitation( $email, $project_name, $inviter_name, $invitation_url ) {
        $subject = sprintf( __( 'You\'ve been invited to %s', 'projectfob' ), $project_name );

        $message = sprintf(
            __( '<p><strong>%s</strong> has invited you to collaborate on <strong>%s</strong>.</p>', 'projectfob' ),
            $inviter_name,
            $project_name
        );
    ...
    public static function send_assignment_notification( $user_id, $item_type, $item_title, $assigner_name, $item_url ) {
        ...
        $subject = sprintf( __( 'You were assigned: %s', 'projectfob' ), $item_title );

        $message = sprintf(
            __( '<p><strong>%s</strong> assigned you a %s:</p>', 'projectfob' ),
            $assigner_name,
            $item_type
        );
    ...
    private static function wrap_in_template( $content ) {
        $company_name = get_option( 'pfob_company_name' );
