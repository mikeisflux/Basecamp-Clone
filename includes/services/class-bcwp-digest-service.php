...
class BCWP_Digest_Service {
    ...
    public static function init() {
        ...
        add_action( 'bcwp_daily_digest', array( __CLASS__, 'send_daily_digests' ) );
        add_action( 'bcwp_weekly_digest', array( __CLASS__, 'send_weekly_digests' ) );

        // Schedule events if not already scheduled
        if ( ! wp_next_scheduled( 'bcwp_daily_digest' ) ) {
            wp_schedule_event( strtotime( 'tomorrow 9:00am' ), 'daily', 'bcwp_daily_digest' );
        }

        if ( ! wp_next_scheduled( 'bcwp_weekly_digest' ) ) {
            wp_schedule_event( strtotime( 'next Monday 9:00am' ), 'weekly', 'bcwp_weekly_digest' );
        }
    }
    ...
    public static function send_daily_digests() {
        $users = get_users( array(
            'meta_key'   => 'bcwp_digest_frequency',
            'meta_value' => 'daily',
        ) );
    ...
    private static function get_recent_messages( $project_id, $since ) {
        ...
        return $wpdb->get_results( $wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}bcwp_messages
            WHERE project_id = %d
            AND created_at >= %s
    ...
    private static function get_recent_todos( $project_id, $since ) {
        ...
        return $wpdb->get_results( $wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}bcwp_todos
            WHERE project_id = %d
            AND created_at >= %s
            AND is_completed = 0
            ORDER BY created_at DESC
    ...
    private static function get_upcoming_events( $project_id, $days_ahead = 7 ) {
        ...
        return $wpdb->get_results( $wpdb->prepare(
            "SELECT * FROM {$wpdb->prefix}bcwp_events
            WHERE project_id = %d
                        ...
                        <div class="section">
                            ...
                            <?php foreach ( $data['new_messages'] as $item ) : ?>
                                <div class="item">
                                    ...
                                    <div class="item-meta">
                                        By <?php echo esc_html( BCWP_Auth_Service::get_user_display_name( $item['message']->created_by ) ); ?>
                                ...
                                <div class="item">
                                    ...
                                    <div class="item-meta">
                                        <?php if ( $item['todo']->assigned_to ) : ?>
                                            Assigned to <?php echo esc_html( BCWP_Auth_Service::get_user_display_name( $item['todo']->assigned_to ) ); ?>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            <?php endforeach; ?>
    ...
    public static function get_user_preference( $user_id ) {
        return get_user_meta( $user_id, 'bcwp_digest_frequency', true ) ?: 'none';
    }
    ...
    public static function set_user_preference( $user_id, $frequency ) {
        ...
        return update_user_meta( $user_id, 'bcwp_digest_frequency', $frequency );
    }
    ...
    public static function clear_schedules() {
        $timestamp = wp_next_scheduled( 'bcwp_daily_digest' );
        if ( $timestamp ) {
            wp_unschedule_event( $timestamp, 'bcwp_daily_digest' );
        }

        $timestamp = wp_next_scheduled( 'bcwp_weekly_digest' );
        if ( $timestamp ) {
            wp_unschedule_event( $timestamp, 'bcwp_weekly_digest' );
        }
    }
}
