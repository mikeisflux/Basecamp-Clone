...
class PFOB_Todo {
    ...
    public static function create_list( $data ) {
        ...
        return PFOB_Database::insert( 'todo_lists', $data );
    }

    public static function get_list( $list_id ) {
        return PFOB_Database::get_row( 'todo_lists', array( 'id' => $list_id ) );
    }
    ...
    public static function update_list( $list_id, $data ) {
        return PFOB_Database::update( 'todo_lists', $data, array( 'id' => $list_id ) );
    }

    public static function delete_list( $list_id ) {
        return PFOB_Database::delete( 'todo_lists', array( 'id' => $list_id ) );
    }
    ...
    public static function create_item( $data ) {
        ...
        $item_id = PFOB_Database::insert( 'todo_items', $data );

        if ( $item_id && ! empty( $data['assignee_id'] ) ) {
            // Send notification to assignee
            $list = self::get_list( $data['list_id'] );
            PFOB_Database::insert( 'notifications', array(
    ...
    public static function get_item( $item_id ) {
        return PFOB_Database::get_row( 'todo_items', array( 'id' => $item_id ) );
    }
    ...
    public static function update_item( $item_id, $data ) {
        ...
        return PFOB_Database::update( 'todo_items', $data, array( 'id' => $item_id ) );
    }

    public static function delete_item( $item_id ) {
        return PFOB_Database::delete( 'todo_items', array( 'id' => $item_id ) );
    }
    ...
    public static function get_user_assigned_items( $user_id, $project_id = null ) {
        ...
        $sql = "SELECT ti.*, tl.name as list_name, tl.project_id
                FROM " . PFOB_Database::get_table_name( 'todo_items' ) . " ti
                INNER JOIN " . PFOB_Database::get_table_name( 'todo_lists' ) . " tl
                ON ti.list_id = tl.id
