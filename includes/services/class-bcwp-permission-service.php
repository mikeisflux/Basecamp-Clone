...
class BCWP_Permission_Service {
    ...
    public static function can_access_project( $project_id, $user_id = null ) {
        ...
        return BCWP_Project::is_member( $project_id, $user_id );
    }
    ...
    public static function has_project_role( $project_id, $user_id, $role ) {
        $member = BCWP_Database::get_row( 'project_members', array(
    ...
    public static function can_edit_content( $type, $content_id, $user_id = null ) {
        ...
        switch ( $type ) {
            case 'message':
                $content = BCWP_Message::get( $content_id );
                $author_field = 'author_id';
                break;
            case 'todo':
                $content = BCWP_Todo::get_item( $content_id );
                $author_field = 'created_by';
                break;
            case 'document':
                $content = BCWP_Document::get( $content_id );
                $author_field = 'uploaded_by';
                break;
            case 'chat':
                $content = BCWP_Chat::get_message( $content_id );
                $author_field = 'user_id';
                break;
            case 'event':
                $content = BCWP_Event::get( $content_id );
                $author_field = 'created_by';
                break;
            case 'card':
                $content = BCWP_Card::get_card( $content_id );
                $author_field = 'created_by';
                break;
        }
    ...
    public static function require_project_access( $project_id ) {
        if ( ! self::can_access_project( $project_id ) ) {
            wp_die( __( 'You do not have permission to access this project.', 'basecamp-wp-pro' ), 403 );
        }
    }
}
