...
class BCWP_Search_Endpoint {

    /**
     * Register routes
     */
    public function register_routes() {
        register_rest_route( 'bcwp/v1', '/search', array(
            'methods'             => 'GET',
    ...
    public function search( $request ) {
        ...
        $results = BCWP_Search_Service::search( $query, $project_id, $types );
    ...
    private function format_project( $project ) {
        return array(
            'id'    => $project->id,
            'title' => $project->name,
            'url'   => home_url( '/basecamp/projects/' . $project->slug ),
            'type'  => 'project',
            'icon'  => '📁',
        );
    }
    ...
    private function format_message( $message ) {
        $project = BCWP_Project::get( $message->project_id );

        return array(
            'id'      => $message->id,
            'title'   => $message->title,
            'excerpt' => wp_trim_words( $message->content, 20 ),
            'url'     => home_url( "/basecamp/projects/{$project->slug}/messages/{$message->id}" ),
            'type'    => 'message',
            'icon'    => '💬',
    ...
    private function format_todo( $todo ) {
        $project = BCWP_Project::get( $todo->project_id );
    ...
    private function format_document( $document ) {
        $project = BCWP_Project::get( $document->project_id );

        return array(
            'id'       => $document->id,
            'title'    => $document->name,
            'excerpt'  => $document->description ? wp_trim_words( $document->description, 20 ) : '',
            'url'      => home_url( "/basecamp/projects/{$project->slug}/documents" ),
            'type'     => 'document',
            'icon'     => '📄',
            'project'  => $project->name,
    ...
    private function format_event( $event ) {
        $project = BCWP_Project::get( $event->project_id );
    ...
    private function format_card( $card ) {
        $project = BCWP_Project::get( $card->project_id );

        return array(
            'id'      => $card->id,
            'title'   => $card->title,
            'excerpt' => $card->description ? wp_trim_words( $card->description, 20 ) : '',
            'url'     => home_url( "/basecamp/projects/{$project->slug}/cards" ),
            'type'    => 'card',
            'icon'    => '🎴',
            'project' => $project->name,
        );
